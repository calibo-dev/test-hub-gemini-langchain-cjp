...
def build_metadata_fetching_agent():
    stages_config = get_stages_config()
    stage_cfg = stages_config.get("metadata_fetching_agent")

    if not stage_cfg:
        raise ValueError("Missing 'metadata_fetching_agent' configuration in stages.yaml")

    tools = get_tools("metadata_fetching_agent") if stage_cfg.get("use_tools", True) else []
    system_prompt = build_stage_system_prompt(stage_cfg)
    model_config = stage_cfg.get("model")
    primary_attempt, fallback_attempts = get_model_attempts("metadata_fetching_agent", model_config)
